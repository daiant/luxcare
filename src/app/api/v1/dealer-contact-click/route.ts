...
export async function POST(req: Request) {
  ...
  if (!body.dealer_id) return new Response(JSON.stringify({ error: 'no dealer' }), { status: 400 });
  ...
  // await sql`INSERT INTO dealer_click_events (dealer, event_type, additional_info) VALUES (${body.dealer}, ${DealerEventType.ClickEvent}, ${body.customer_location});`;
	...
	VALUES ('${body.dealer_id}', 1, '${JSON.stringify(body)}')
	`)
	const getPhone = async (dealer_id: number) => {
		const queryResult = await makeQuery(`SELECT contact_phone from dealers where id = ${dealer_id}`);

		return queryResult[0]?.contact_phone;
	}

  return new Response(JSON.stringify({ value: await getPhone(body.dealer_id) }));
}
